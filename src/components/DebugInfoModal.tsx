...
export const DebugInfoModal: React.FC<DebugInfoModalProps> = ({ isOpen, onClose, session }) => {
  ...
  const generateMarkdownReport = () => {
    ...
    const currentQ =
      questions && questions.length > currentQuestionIndex ? questions[currentQuestionIndex] : null;
    ...
    if (analysis?.answerScore) {
      ...
      if (analysis.dimensionScores) {
        analysis.dimensionScores?.forEach((ds) => {
          const dim = blueprint?.scoringModel?.dimensions?.find(
            (d) => d.name === ds.dimensionId || d.id === ds.dimensionId
          );
      ...
      const totalWeightedScore =
        analysis.dimensionScores?.reduce((sum, ds) => {
          const dim = blueprint?.scoringModel?.dimensions?.find(
            (d) => d.name === ds.dimensionId || d.id === ds.dimensionId
          );
    ...
    if (allAnswered.length > 0) {
      const avgScore = Math.round(
        allAnswered.reduce((sum, a) => sum + (a.analysis?.answerScore || 0), 0) / allAnswered.length
      );
      ...
      <div className="w-full max-w-4xl h-[90vh] flex flex-col bg-zinc-900/95 border border-cyan-500/20 shadow-2xl rounded-xl overflow-hidden">
        ...
        <div ref={contentRef} className="flex-1 overflow-auto p-6 custom-scrollbar bg-black/40">
          <ReactMarkdown
            components={{
              h1: ({ children }) => (
                <h1 className="text-2xl font-bold text-zinc-400 mb-4 pb-2">{children}</h1>
              ),
              h2: ({ children }) => (
                <h2 className="text-xl font-bold text-blue-400 mt-6 mb-3">{children}</h2>
              ),
              h3: ({ children }) => (
                <h3 className="text-lg font-semibold text-amber-500 mt-4 mb-2">{children}</h3>
              ),
              h4: ({ children }) => <h4 className="text-blue-400 mt-3 mb-1">{children}</h4>,
              p: ({ children }) => (
                <p className="text-zinc-200 text-sm leading-relaxed mb-2">{children}</p>
              ),
              strong: ({ children }) => (
