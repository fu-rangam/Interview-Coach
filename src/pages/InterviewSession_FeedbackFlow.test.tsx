...
vi.mock('../components/ui/glass/SessionLoader', () => ({
    SessionLoader: ({ onTransitionComplete }: { onTransitionComplete: () => void }) => {
        React.useEffect(() => { onTransitionComplete(); }, [onTransitionComplete]);
...
vi.mock('../components/session/TipsAndTranscriptContent', () => ({ TipsAndTranscriptContent: () => <div /> }));
vi.mock('../components/ui/glass/GlassCard', () => ({ GlassCard: ({ children }: any) => <div>{children}</div> }));
vi.mock('../components/ui/glass/GlassButton', () => ({ GlassButton: ({ onClick, children, ...props }: any) => <button onClick={onClick} {...props}>{children}</button> }));
...
describe('InterviewSession Feedback Flow', () => {
    ...
    it('shows loader then modal if analysis is still pending when "Now" is clicked', async () => {
        ...
        analyzeAnswerMock.mockReturnValue(new Promise(resolve => { resolveAnalysis = resolve; }));
        ...
        const saveAnswerMock = vi.fn((qId, ans) => { mutableSession.answers[qId] = ans; });
        ...
        // Record & Submit
        await React.act(async () => { fireEvent.click(screen.getByRole('button', { name: "Toggle Recording" })); });
        ...
        await React.act(async () => { fireEvent.click(screen.getByText('Submit Answer')); });
        ...
        await waitFor(() => expect(screen.getByTestId('feedback-modal')).toBeInTheDocument(), { timeout: 4000 });
